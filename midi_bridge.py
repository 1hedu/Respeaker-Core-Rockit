...
# -*- coding: utf-8 -*-
"""
Rockit MIDI Bridge - Minimal TCP-to-MIDI forwarder
Listens on port 8090 for HTTP requests, forwards to TCP MIDI on port 50000
"""
...
def send_raw_midi(status, data1, data2):
    """Send 3-byte MIDI message via TCP"""
    try:
...
class MIDIBridgeHandler(BaseHTTPRequestHandler):
    def do_OPTIONS(self):
...
if __name__ == '__main__':
    print("MIDI Bridge starting on port 8090...")
    print("Forwarding to TCP MIDI on %s:%d" % (TCP_HOST, TCP_PORT))
