...
void paraphonic_init(void) {
    para_state.mode = MODE_ROUND_ROBIN;  // Default: paraphonic round robin
 ...
 * Most recent note plays on both oscillators (detuned unison)
 */
static void allocate_voices_monophonic(g_setting *p_global_setting) {
    // Just play most recent note on OSC_1 (OSC_2 follows via detune)
    ...
    // OSC_2 will follow OSC_1 via normal Rockit detuning
...
static void allocate_low_note_priority(g_setting *p_global_setting) {
    ...
    if (para_state.stack_size == 1) {
        ...
        p_global_setting->auc_midi_note_index[OSC_1] = note;
        p_global_setting->auc_midi_note_index[OSC_2] = note;
        p_global_setting->uc_note_velocity = velocity;
        ...
        for (unsigned char i = 0; i < voices_to_assign; i++) {
            ...
            if (i == 0) {
                ...
                p_global_setting->auc_midi_note_index[OSC_2] = sorted_notes[i];
    ...
    if (para_state.stack_size == 1) {
        ...
        for (unsigned char i = 0; i < voices_to_assign; i++) {
            ...
            if (i == 0) {
                ...
                p_global_setting->auc_midi_note_index[OSC_2] = para_state.note_stack[stack_idx];
            } else if (i == 2 && para_state.ef101d_enabled) {
                ...
                for (unsigned char v = 0; v < max_voices && assigned < max_voices; v++) {
                    if (!para_state.voices[v].active) {
                        ...
                        if (v == 0) {
                            ...
                            p_global_setting->auc_midi_note_index[OSC_2] = para_state.note_stack[i];
...
static void clear_voice(unsigned char voice_idx, g_setting *p_global_setting) {
    ...
    // IMPORTANT: Actually silence the hardware oscillator!
    // Set it to note 0 (very low) so it doesn't play old note
    if (voice_idx == 0) {
        p_global_setting->auc_midi_note_index[OSC_1] = 0;
    } else if (voice_idx == 1) {
        p_global_setting->auc_midi_note_index[OSC_2] = 0;
    } else if (voice_idx == 2 && para_state.last_ef101_note != 0) {
...
// CC 103: Mode selection (0-4 for all 5 modes)
void paraphonic_handle_enable_cc(unsigned char value, g_setting *p_global_setting) {
    // Map CC value 0-127 to modes 0-4
    // 0-25: Mono, 26-50: Low, 51-75: Last, 76-100: RR, 101-127: High
    if (value < 26) {
        paraphonic_set_mode(MODE_MONOPHONIC, p_global_setting);
    } else if (value < 51) {
...
// CC 102: Select paraphonic mode (when paraphonic is enabled)
void paraphonic_handle_mode_cc(unsigned char value, g_setting *p_global_setting) {
    if (para_state.mode == MODE_MONOPHONIC) {
        // Ignore CC 102 when in mono mode
        return;
    }
...
// CC 104: Toggle EF-101D (third voice)
